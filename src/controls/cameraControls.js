...
export default class CameraControls extends EventDispatcher {
	...
	constructor( camera, domElement, options = {} ) {
		...
		if ( ! this._domElement || options.ignoreDOMEventListeners ) {
			...
			function dragging( event ) {
				...
				if(deltaX < 0) window.touch = 'left'
				else if(deltaX > 0)window.touch = 'right'
  ...
  rotateToVector(cameraDirection, targetVector, enableTransition) {
    ...
    console.log('enableTransition', enableTransition)
		var sph1 = new THREE.Spherical()
		var sph2 = new THREE.Spherical()
    var cameraXOZ = new THREE.Vector3(cameraDirection.x, 0, cameraDirection.z)
    var targetXOZ = new THREE.Vector3(targetVector.x, 0, targetVector.z)
    var cameraYOZ = new THREE.Vector3(0, cameraDirection.y, cameraDirection.z)
    var targetYOZ = new THREE.Vector3(0, targetVector.y, targetVector.z)
    var azimuthAngle = cameraXOZ.angleTo(targetXOZ)
    var polarAngle = cameraYOZ.angleTo(targetYOZ)
    // var azimuthAngle = Math.acos(cameraXOZ.dot(targetXOZ) / (cameraXOZ.length() * targetXOZ.length()))
    // var polarAngle = Math.acos(cameraYOZ.dot(targetYOZ) / (cameraYOZ.length() * targetYOZ.length()))
    console.log('ccccccc', azimuthAngle, polarAngle)
    // var XOZVector1 = new THREE.Vector3().clone(cameraXOZ)
    // var XOZVector2 = new THREE.Vector3().clone(targetXOZ)
    // var YOZVector1 = new THREE.Vector3().clone(cameraYOZ)
    // var YOZVector2 = new THREE.Vector3().clone(targetYOZ)
    var azimuthDirection = cameraXOZ.clone().cross(targetXOZ.clone())
    var polarDirection = cameraYOZ.clone().cross(targetYOZ.clone())
    if (azimuthDirection.y < 0) {
      azimuthAngle = -azimuthAngle
    }
    if (polarDirection.x < 0) {
      polarAngle = -polarAngle
    }
		var polar1 = sph1.setFromVector3(cameraDirection)
		var polar2 = sph2.setFromVector3(targetVector)
		console.log('polar', polar1, polar2)
    console.log('cameraYOZ', cameraYOZ, 'targetYOZ', targetYOZ)
    console.log('azimuthAngle', azimuthAngle, 'polarAngle', polarAngle)
    this.rotate(azimuthAngle, -(polar2.phi - polar1.phi), enableTransition)
  }
